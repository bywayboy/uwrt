...
static void automem_ensure_size(automem_t * pmem, unsigned int size){
	unsigned int newbuffersize = 0;
	if (pmem->buffersize - pmem->size < size)
		newbuffersize = pmem->buffersize + size * 500;
