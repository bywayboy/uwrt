...
void uci_add_delta(struct uci_context *ctx, struct uci_list *list, int cmd, const char *section, const char *option, const char *value);
void uci_free_delta(struct uci_delta *h);
struct uci_package *uci_alloc_package(struct uci_context *ctx, const char *name);
...
void uci_getln(struct uci_context *ctx, int offset);
...
struct uci_element *uci_lookup_list(struct uci_list *list, const char *name);
void uci_fixup_section(struct uci_context *ctx, struct uci_section *s);
void uci_free_package(struct uci_package **package);
struct uci_element *uci_alloc_generic(struct uci_context *ctx, int type, const char *name, int size);
void uci_free_element(struct uci_element *e);
struct uci_element *uci_expand_ptr(struct uci_context *ctx, struct uci_ptr *ptr, bool complete);

int uci_load_delta(struct uci_context *ctx, struct uci_package *p, bool flush);

static __inline bool uci_validate_package(const char *str)
{
	return uci_validate_str(str, false);
}

static __inline bool uci_validate_type(const char *str)
{
	return uci_validate_str(str, false);
}

static __inline bool uci_validate_name(const char *str)
{
	return uci_validate_str(str, true);
}

/* initialize a list head/item */
static __inline void uci_list_init(struct uci_list *ptr)
{
	ptr->prev = ptr;
	ptr->next = ptr;
}
...
static __inline void uci_list_insert(struct uci_list *list, struct uci_list *ptr)
{
	list->next->prev = ptr;
	ptr->prev = list;
	ptr->next = list->next;
...
static __inline void uci_list_add(struct uci_list *head, struct uci_list *ptr)
{
	/* NB: head->prev points at the tail */
	uci_list_insert(head->prev, ptr);
}

static __inline void uci_list_del(struct uci_list *ptr)
{
 ...
 */
		...
		#define UCI_THROW(ctx, err) do { 	\
				fprintf(stderr, "Exception: %s in %s, %s:%d\n", #err, __FUNCTION__, __FILE__, __LINE__); \
				longjmp(ctx->trap, err); 	\
			} while (0)
	#else
		#define UCI_THROW(ctx, err) do { 	\
				fprintf(stderr, "Exception: %s in %s:%d\n", #err,  __FILE__, __LINE__); \
				longjmp(ctx->trap, err); 	\
			} while (0)
	#endif
#else
