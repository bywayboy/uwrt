...
#include "list.c"
 ...
 const char *uci_confdir = UCI_CONFDIR;
 const char *uci_savedir = UCI_SAVEDIR;
...
struct uci_context * uci_alloc_context(void)
{
	...
	ctx->savedir = _strdup(getenv("TEMP"));//(char *) uci_savedir;
#else
	ctx->savedir = UCI_SAVEDIR;
#endif
...
uci_get_errorstr(struct uci_context *ctx, char **dest, const char *prefix)
{
	...
	case UCI_ERR_PARSE:
		if (ctx->pctx) {
#if defined(_WIN32) || defined(_WIN64)
			_snprintf(error_info, sizeof(error_info) - 1, " (%s) at line %d, byte %d", (ctx->pctx->reason ? ctx->pctx->reason : "unknown"), ctx->pctx->line, ctx->pctx->byte);
#else
