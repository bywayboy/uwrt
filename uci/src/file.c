 ...
 */
static __inline bool parse_backslash(struct uci_context *ctx, char **str)
{
	/* skip backslash */
	*str += 1;
...
static __inline void addc(char **dest, char **src)
{
	**dest = **src;
	*dest += 1;
	*src += 1;
...
static void uci_parse_config(struct uci_context *ctx, char **str)
{
	...
	*str += strlen(*str) + 1;
	...
	if (!name || !name[0]) {
		...
		do {
			ctx->nested = true;
			uci_add_section(ctx, pctx->package, type, &pctx->section);
		} while (0);
	} else {
 ...
 */
static __inline char *get_filename(char *path)
{
	char *p;
...
struct uci_backend uci_file_backend ={
/*element*/
	{
	/*list*/
		{
			&uci_file_backend.e.list,
			&uci_file_backend.e.list
		},
		UCI_TYPE_BACKEND,
		"file"
	},
	uci_list_config_files,
	uci_file_load,uci_file_commit,
	&uci_file_backend,
	NULL
};
/*

UCI_BACKEND(uci_file_backend, "file",
	.load = uci_file_load,
