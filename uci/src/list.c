...
static __inline void uci_list_fixup(struct uci_list *ptr)
{
	ptr->prev->next = ptr;
	ptr->next->prev = ptr;
}
...
uci_alloc_generic(struct uci_context *ctx, int type, const char *name, int size)
{
	...
	ptr = (void*)uci_malloc(ctx, datalen);
...
uci_lookup_ptr(struct uci_context *ctx, struct uci_ptr *ptr, char *str, bool extended)
{
	...
	if (!e) {
		...
		do {
			ctx->internal = true;
			uci_load(ctx, ptr->package, &ptr->p);
		} while (0);
		if (!ptr->p)
			goto notfound;
		ptr->last = &ptr->p->e;
...
int uci_set(struct uci_context *ctx, struct uci_ptr *ptr)
{
	...
	if (!ptr->value[0]) {
		...
		char *s = (char*)uci_strdup(ctx, ptr->value);
